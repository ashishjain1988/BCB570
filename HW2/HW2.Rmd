---
title: "BCB570 Assignment 1"
author: "Ashish Jain"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
    echo = TRUE,
    message = FALSE,
    warning = FALSE
)
#install.packages("igraph")
```

#Question 1:

```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
library("ggplot2")
degreeDistribution<-c(2,4,4,4,4,5,4,3,2)
degreeProb<-c(0,2/9,1/9,5/9,1/9)
degrees<-c(1:5)
ccDistribution<-c(1,0.5,0.5,0.5,0.5,2/5,0.5,2/3,1)
data<-data.frame(degree=degrees,degreeProbabilty=degreeProb,cc=c(0,1,2/3,0.5,2/5))
ggplot(data=data, aes(x=degrees,y=degreeProbabilty))+
  geom_point() + labs(y="P(k)", x="Degree (k)")

ggplot(data=data, aes(x=degrees,y=cc)) +
    geom_point() + labs(y="C(k)", x="Degree(k)")

```

#Question 2:

![](https://upload.wikimedia.org/wikipedia/commons/8/82/US-ASCII_code_chart.png)

#Question 3:

#Question 4:
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
library("igraph")
completegraph <- graph(combn(1:5,2),directed=FALSE)
plot.igraph(completegraph)
```

#Question 5:
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
library(pander)
graphDetails<-function(edgeFile)
{
  graph<-as.matrix(read.table(edgeFile))
  graphGraph<-graph.data.frame(graph,directed = FALSE)
  graphDetails<-c(length(V(graphGraph)), length(E(graphGraph)), graph.density(graphGraph),diameter(graphGraph), radius(graphGraph), transitivity(graphGraph, type = "average"),transitivity(graphGraph, type = "global"),mean_distance(graphGraph), toString(head(order(igraph::degree(graphGraph),decreasing = TRUE),5)), toString(head(order(igraph::betweenness(graphGraph),decreasing = TRUE),5)), toString(intersect(head(order(igraph::degree(graphGraph),decreasing = TRUE),5), head(order(igraph::betweenness(graphGraph),decreasing = TRUE),5))))
  return(graphDetails)
}

details<-c("Number of Nodes","Number of Edges","Graph Density","Diameter","Radius","Average Clustering Coefficent","Global Clustering Coefficent","Average Shortest Path","Top 5 Central Nodes(Hightest Degree)","Top 5 Central Nodes (Betweeness)","Common Nodes")
path<-"https://raw.githubusercontent.com/ashishjain1988/BCB570/master/HW2/"
graphADetails<-graphDetails(paste0(path,"graphA.txt"))
graphBDetails<-graphDetails(paste0(path,"graphB.txt"))
graphCDetails<-graphDetails(paste0(path,"graphC.txt"))

table<-data.frame(Headings=details,Graph_A=graphADetails,Graph_B=graphBDetails,Graph_C=graphCDetails)
knitr::kable(table,caption = "Graph Details")
```

i). As from the table, the number of nodes for graph A and C is close to the number of edges. Apart from that, the graph density which is the ratio of the edges and number of possible edges is very low. From these two parameters we can say that Graph A and C are sparse. In the case of graph B, the number of edges is not close to either the number of nodes or $nodes^{2}$. So, from that we can say that this graph is somewhere in-between.

ii). The diameter and the radius of the graphs is shown in table 1.

iii). The global clustering coefficient is the ratio of the number of the traingles or closed triplets and the number of connected triples of vertices in the graph. On the other hand, the average clustering coefficient is the average of the local clustering coefficient of all the vertices in the graph. The local clustering coefficient is the ratio of the triangles connected to the vertex and the triples centered on the vertex. From their definations, the difference between them is that global clustering coefficient puts more weight on the high degree nodes, while the local clustering coefficient places more weight on the low degree nodes.

iv).The average shortest path length of the graphs is shown in table 1.

v). Degree centrality and Betweenness centrality have been used to find central nodes.
Degree centrality measures the number of edges a given vertex.
Betweeness centrality gives a higher score to a node if it connects to many high score nodes.
There is a very high overlap between the central nodes as measured by both the centrality measures. 

These nodes are selected as central nodes, because they are in the shortest path between several nodes and have easier access to all other nodes. They also form edges with other high scoring vertices.

#Question 6:
To check for small-world property of the networks, we can do the following:

i). Compute degree distribution of the network.

ii). Try to fit a power law distribution to the degree distribution.

iii). Compare clustering coefficients of the network with that of a random network with
same number of nodes and edges.

iv). Compare average path lengths of the network with that of a random network with 
same number of nodes and edges.

v). For a small world network:


#Question 7:
To check for scale-free property of the networks, we can do the following:

i). Plot degree distribution vs # of nodes (log-log scale) to check for a linear decreasing 
relation.

ii). Plot clustering coefficient vs # of nodes (log-log scale) to check for a constant line.

iii). Check if there are few hubs.

iv). Try to fit a power law distribution to the degree distribution and if p<0.05, the network is not  cale free.

v). Take a subset of the network and check if it retains the properties of the full network.

vi). Expand the network (with more real data or accurately simulated data) and check if it retains the properties of the full network.